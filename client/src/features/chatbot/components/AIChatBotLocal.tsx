...
import { MessageCircle, Send, Plus, Trash2, ChevronLeft, ChevronRight, Bot, User, Zap, AlertTriangle, CheckCircle, XCircle, Settings, History, Wifi, WifiOff } from 'lucide-react';
import { cn } from '@/lib/utils';
import { LocalAIModelStatus } from './ai/LocalAIModelStatus';
import { ChatDeleteConfirmation } from '@/shared/components/ui/delete-confirmation-dialog';
import { OllamaConfigPanel } from '@/features/settings';
...
function AIChatBotLocal() {
  ...
  const [lmStudioHealth, setLMStudioHealth] = useState<ServerHealth>({ isOnline: false, models: [] });
  ...
  // Use consolidated Ollama configuration
  const {
    config: ollamaConfig,
  ...
  // Use consolidated LM Studio configuration
  const { 
  ...
  useEffect(() => {
    localStorage.setItem('chatHistorySidebarOpen', JSON.stringify(showHistory));
  }, [showHistory]);
  ...
  const sendMessage = async () => {
    ...
    if (!serverHealth.isOnline) {
      toast({
        ...
        description: `${aiProvider} server is not available. Please check your connection.`,
        variant: "destructive"
      });
      return;
    }

    // Additional check for LM Studio model availability
    ...
    try {
      const currentModel = aiProvider === 'ollama' 
        ? ollamaConfig.model 
        : (lmStudioModels && lmStudioModels.length > 0 ? lmStudioModels[0] : null);
        
      if (!currentModel) {
        throw new Error(`No model available for ${aiProvider}`);
      }

      const response = await securePost('/api/chat/message', {
        message: inputMessage,
        provider: aiProvider,
        model: currentModel,
        context: updatedSession.messages.slice(-10)
      });
      ...
      setChatHistory(prev => {
        ...
        // Re-sort to ensure newest chats are first
  ...
  // Allow Enter key to create new lines without sending the message
  ...
  const getCurrentServerHealth = () => {
    return aiProvider === 'ollama' ? 
      { isOnline: ollamaConnected, models: ollamaModels, error: ollamaConnectionStatus.error } :
  ...
  const handleModelChange = (model: string) => {
    if (aiProvider === 'ollama') {
      // Update the model in the config which will be saved to localStorage by the useOllamaConfig hook
      updateConfig({ model });
    }
    // For LM Studio, we don't need to do anything special as it will use the first available model
  };
        ...
        <div className="bg-background border-b border-border p-4 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <LocalAIModelStatus
              ...
              currentModel={aiProvider === 'ollama' ? ollamaConfig.model : (lmStudioModels && lmStudioModels.length > 0 ? lmStudioModels[0] : 'No model')}
              sessionInfo={currentSession ? `${currentSession.messages.length} messages` : 'No active chat'}
              ollamaConnected={ollamaConnected}
              ollamaModels={ollamaModels}
              ollamaConnectionStatus={ollamaConnectionStatus.error || (ollamaConnectionStatus.connected ? 'Connected' : 'Disconnected')}
          ...
          {currentSession ? (
                    ...
                    >
                      <div className="flex items-start gap-2">
                        {message.role === 'assistant' && (
                          <Bot className="w-5 h-5 mt-0.5 text-brand-600 flex-shrink-0" />
                        )}
                        {message.role === 'user' && (
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center max-w-md mx-auto p-6">
                <Bot className="w-16 h-16 text-brand-600 mx-auto mb-4" />
                <h3 className="text-xl font-semibold text-foreground mb-2">Welcome to AI ChatBot Local</h3>
                <p className="text-muted-foreground mb-6">
                  Start a conversation with your local AI models. Choose between Ollama and LM Studio for different AI experiences.
                </p>
                <Button
                  onClick={createNewChat}
                ...
                <div className="flex gap-2 relative">
                  <div className="relative flex-1 border border-input rounded-md focus-within:border-ring transition-colors duration-200">
                    <textarea
                      ...
                      placeholder={`Message ${aiProvider === 'ollama' ? 'Ollama' : 'LM Studio'}...`}
                      disabled={isLoading || !getCurrentServerHealth().isOnline}
                      rows={1}
                      className={cn(
                  ...
                  <Button
                    onClick={sendMessage}
                    disabled={!inputMessage.trim() || isLoading || !getCurrentServerHealth().isOnline}
